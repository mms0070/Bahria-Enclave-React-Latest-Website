// Billing portal client-side logic — same functions as billing-portal.js, exported as ESM.

export function hoverIn(el, styles) {
  if (el.dataset.origStyle === undefined) el.dataset.origStyle = el.getAttribute('style') || '';
  Object.assign(el.style, styles);
}
export function hoverOut(el) {
  if (el.dataset.origStyle !== undefined) el.setAttribute('style', el.dataset.origStyle);
}

// Called from App useEffect after mount
...
export function setElectric() { billType = 'electricity'; updateTabStyles(); }
export function setMaint() { billType = 'maintenance'; updateTabStyles(); }

export function onRef() {
...
export function areaHintFromPrefix(prefix) {
...
export function payViaKuickPayRef(ref) {
...
export function payViaKuickPay() {
